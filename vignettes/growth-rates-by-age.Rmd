...
* Case data is pillar 1 & 2 test data from the line list provided by PHE. We excluded Pillar 2 cases marked a "asymptomatic". These data are potentially affected by saturation when test demand outstrips availability. This bias is discussed below and see `r ref$sfig("SymptomaticVsAsymptomatic")` for details.
* Deaths data is from the line list provided by PHE.
* Maps & demographic information is from the ONS, and based on mid year 2019 estimates.
...
* Growth dynamics within regional age groups suggest the timing of the second wave has been driven by infections in school an university age people, but that growth was established in these groups well before the return their respective terms started.

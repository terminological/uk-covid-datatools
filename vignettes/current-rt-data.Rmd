---
title: "Current $R_t$: Data selection and QA"
author: "Rob Challen"
...
p = tsp$plotIncidenceQuantiles(currentRt$rt, denominatorExpr = population/1000000, events = events, colour=statistic, ylim=c(0,150))+
  ...
  scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+ylab("per 1M per day")
p %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/current-rt/data/Fig20_AllGeogsAllModels")
...
p2 = tsp$plotRt(
  ...
  theme(legend.position = "bottom")
p2 %>% 
  standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/current-rt/data/Fig21_Rt7_AllGeogsAllModels")
...
p3 = tsp$plotWindowedGrowthRate(
  ...
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p3 %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/current-rt/data/Fig22_GrowthRate7_AllGeogsAllModels")
...
p2 = tsp$plotRt(
  ...
  theme(legend.position = "bottom")
p2 %>% 
  standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/current-rt/data/Fig23_Rt28_AllGeogsAllModels")
...
p3 = tsp$plotWindowedGrowthRate(
  ...
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p3 %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/current-rt/data/Fig24_GrowthRate28_AllGeogsAllModels")
...
```{r}
  ...
  exportSPIM7day = bind_rows(
    currentRt$rt %>% 
      ...
      convertRtToSPIM(name,modelExpr = paste0("EpiEstim/",source), version="7 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/4NationsCases", version="7 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/Deaths", version="7 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/Triage", version="7 day"),
    
    currentRt$rt %>% 
      ungroup() %>%
      filter(source %in% c("4NationsCases","Deaths","Triage")) %>% 
      filter(!is.na(`Median(R)`)) %>% 
      convertGrowthRateToSPIM(name,modelExpr = paste0("EpiEstim/",source), version="7 day")
  ...
  exportSPIM28Day = bind_rows(
    currentRt$rt28 %>% 
      ...
      filter(source %in% c("4NationsCases","Deaths","Triage")) %>% 
      filter(!is.na(`Median(R)`)) %>% 
      convertRtToSPIM(name,modelExpr = paste0("EpiEstim/",source), version="28 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/4NationsCases", version="28 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/Deaths", version="28 day"),
    tsp$rtConfig %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/Triage", version="28 day"),
    
    currentRt$rt28 %>% 
      ungroup() %>%
      filter(source %in% c("4NationsCases","Deaths","Triage")) %>% 
      filter(!is.na(`Median(R)`)) %>% 
      convertGrowthRateToSPIM(name,modelExpr = paste0("EpiEstim/",source), version="28 day")
    # cfg %>% convertSerialIntervalToSPIM(modelName = "EpiEstim/PHETrackerCases")
  )

...
knit: (function(inputFile, encoding,...) {
  rmarkdown::render(inputFile, encoding = encoding, output_dir = paste0("~/Dropbox/covid19/current-rt/",Sys.Date()), output_file=paste0('current-rt-data-',Sys.Date(),'.pdf')) })
# output: 
...
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(triageCTRY, denominatorExpr = population/1000000, events = events, colour=subgroup, dates=startDate)+
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceQuantiles(finalTriageCTRY, denominatorExpr = population/1000000, events = events, dates=startDate)+facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceQuantiles(casesCTRY, denominatorExpr = population/1000000, events = events, dates=startDate, colour = source)+facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150,500))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceQuantiles(finalCasesCTRY, denominatorExpr = population/1000000, events = events, dates=startDate, nocache=TRUE)+facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150,500))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceRollmean(admissionsCTRY, denominatorExpr = population/1000000, colour=source, events = events, dates=startDate) +
    facet_wrap(vars(name))+
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceQuantiles(finalAdmissionsCTRY, denominatorExpr = population/1000000, events = events, dates=startDate,nocache=TRUE) +facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,2,5,10,20))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceRollmean(deathsCTRY, denominatorExpr = population/1000000, events = events, dates=startDate, colour=source) +facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,2,5,10,20))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(finalDeathsCTRY, denominatorExpr = population/1000000, events = events, dates=startDate,nocache=TRUE) +facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,2,5,10,20))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(casesUK, denominatorExpr = population/1000000, events = events, dates=startDate, colour = source)+scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(finalAdmissionsUK, denominatorExpr = population/1000000, events = events, dates=startDate,nocache=TRUE)+scale_y_continuous(trans="log1p", breaks = c(0,2,5,10,20,50))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(deathsUK, denominatorExpr = population/1000000, events = events, dates=startDate, colour = source,nocache=TRUE)+scale_y_continuous(trans="log1p", breaks = c(0,2,5,10,20))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(triageNHSER, denominatorExpr = population/1000000, events = events, dates=startDate, colour=subgroup)+
    facet_wrap(vars(name))+
    scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(finalTriageNHSER, denominatorExpr = population/1000000, events = events, dates=startDate)+
    facet_wrap(vars(name))+
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(casesNHSER, denominatorExpr = population/1000000, events = events, dates=startDate, colour=subgroup)+
    facet_wrap(vars(name))+
    scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+
    ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p=tsp$plotIncidenceQuantiles(finalCasesNHSER, denominatorExpr = population/1000000, dates=startDate, events = events)+
    facet_wrap(vars(name))+
    scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+
    ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(admissionsNHSER, denominatorExpr = population/1000000, colour=source, events = events, dates=startDate)+
    facet_wrap(vars(name))+
    scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+
    ylab("per 1M per day")
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(finalAdmissionsNHSER, denominatorExpr = population/1000000, events = events, dates=startDate)+
    facet_wrap(vars(name))+
    scale_y_continuous(trans="log1p",breaks=c(0,5,15,50,150))+
    ylab("per 1M per day")
...
<!-- ```{r} -->
<!-- with(currentRt$source, { -->
<!--   p = tsp$plotIncidenceQuantiles(finalIcuAdmissionsNHSER, denominatorExpr = population/1000000, events = events, dates=startDate)+ -->
<!--     facet_wrap(vars(name))+ -->
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(deathsNHSER, denominatorExpr = population/1000000, events = events, dates=startDate)+
    facet_wrap(vars(name))+
...
```{r}
with(currentRt$source, {
  p = tsp$plotIncidenceQuantiles(finalDeathsNHSER, denominatorExpr = population/1000000, events = events, dates=startDate)+facet_wrap(vars(name))+scale_y_continuous(trans="log1p", breaks = c(0,5,15,50,150))+ylab("per 1M per day")
...
```{r}
p = tsp$plotIncidenceQuantiles(currentRt$rt7, denominatorExpr = population/1000000, events = events, dates=startDate, colour=statistic, ylim=c(0,1500))+
...
```{r}
p2 = tsp$plotRt(
    currentRt$rt7 %>% filter(statistic != "icu admission"),
    events = events,
    dates = "2020-11-01",
    colour=statistic,
    rtlim = c(0.5,1.75), ribbons=TRUE)+
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p2 %>%
...
```{r}
currentRt$rt7 %>% filter(statistic != "icu admission" & !is.na(`Mean(R)`) ) %>% group_by(name,statistic) %>% mutate(
...
## Combined data set - 7 day $R_t$ - JEpi

```{r}

p2 = tsp$plotRt(
    currentRt$jepi7,# %>% filter(statistic != "icu admission"), 
    events = events,dates = startDate, 
    colour=statistic, 
    rtlim = c(0.5,1.75), ribbons=TRUE)+
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p2 %>% 
  standardPrintOutput::saveThirdPageFigure("~/Dropbox/covid19/current-rt/data/Fig24_Rt7_Jepi_AllGeogsAllModels")
...
```{r}
p3 = tsp$plotGrowthRate(
    currentRt$jepi7,# %>% filter(statistic != "icu admission"), , 
    colour=statistic, 
    events = events,
    dates = startDate,
    ribbons = TRUE, 
    rlim = c(-0.125,0.125),window=14)+
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p3 %>% standardPrintOutput::saveThirdPageFigure("~/Dropbox/covid19/current-rt/data/Fig25_GrowthRate14_Jepi_AllGeogsAllModels")

```

## Combined data set - 7 day $R_t$ - JEpiSmooth

```{r}

p2 = tsp$plotRt(
    currentRt$jepiSmooth,# %>% filter(statistic != "icu admission"), 
    events = events,dates = startDate, 
    colour=statistic, 
    rtlim = c(0.5,1.75), ribbons=TRUE)+
  facet_wrap(vars(name),ncol=4)+
  theme(legend.position = "bottom")
p2 %>% 
  standardPrintOutput::saveThirdPageFigure("~/Dropbox/covid19/current-rt/data/Fig24_Rt7_JepiSmooth_AllGeogsAllModels")

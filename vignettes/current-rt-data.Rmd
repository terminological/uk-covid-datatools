...
```{r}
currentRt$rt %>% filter(statistic != "icu admission" & !is.na(`Mean(R)`) ) %>% group_by(name,statistic) %>% mutate(Estimate = sprintf("%1.2f (%1.2f; %1.2f)",`Median(R)`,`Quantile.0.025(R)`,`Quantile.0.975(R)`)) %>% select(name,statistic,date,Estimate) %>% filter(date == max(date)) %>% group_by(name) %>% standardPrintOutput::saveTable("~/Dropbox/covid19/current-rt/data/Tab21_Rt7_AllGeogsAllModels",defaultFontSize = 6)
```

...
ends = as.Date(c("2020-04-01","2020-05-01","2020-06-01","2020-07-01"))
...
onsetToTestFit = DistributionFit$new(distributions = c("lnorm","gamma","weibull","nbinom","exp"))$fromUncensoredData(onsetToTest, valueExpr = time, truncate = TRUE, bootstraps = 100)
...
onsetToAdmissionFit = DistributionFit$new(distributions = c("exp","lnorm","gamma","weibull","nbinom"))$fromUncensoredData(onsetToAdmission, valueExpr = time, truncate = TRUE, bootstraps = 100)
plots$onsetToAdmission = onsetToAdmissionFit$plot(xlim = c(0,25))+xlab("time delay")
tables$onsetToAdmission = onsetToAdmissionFit$printDistributionDetail() %>% arrange(aic,dist,param)
```
...
* Deaths assume must happen after onset - capped at 100 days
* Long tail. Large SD.
...
onsetToDeathFit = DistributionFit$new(distributions = c("lnorm","gamma","weibull","nbinom"))$fromUncensoredData(onsetToDeath, valueExpr = time, truncate = TRUE, bootstraps = 100)
plots$onsetToDeath = onsetToDeathFit$plot(xlim = c(0,60))+xlab("time delay")
...
suppTable = bind_rows(tables) %>% ungroup() %>% select(-dist,-bic,-loglik,-mean,-sd,-lower,-upper,-shift) %>% rename(Transition = transition, Parameter= param, AIC= aic, N=n) %>% arrange(AIC) %>% group_by(Transition, Distribution)
suppTable %>% standardPrintOutput::mergeCells()
```
...
## Impact of delays on serial nterval
...
relativeDelay = function(distFit) {
  distFit2 = distFit$clone()
  distFit2$filterModels(aic == min(aic))
  distFit2$generateSamples(sampleExpr = 2000)
  delayDist = distFit2$samples %>%
    mutate(sampleCat = (sampleNumber-1) %/% 1000 + 1, sampleNumber = sampleNumber %% 1000) %>%
    pivot_wider(names_from = sampleCat, values_from = value, names_prefix = "delay") %>% mutate(delayOffset = delay2-delay1)
  return(delayDist)
}

relativeDelays = bind_rows(
  onsetToTestFit %>% relativeDelay(),
  onsetToAdmissionFit %>% relativeDelay(),
  onsetToTestResultFit %>% relativeDelay(),
  onsetToDeathFit %>% relativeDelay()
)

ggplot(relativeDelays %>% filter(delayOffset > -45 & delayOffset < 45), aes(x=delayOffset, colour = transition))+geom_density()

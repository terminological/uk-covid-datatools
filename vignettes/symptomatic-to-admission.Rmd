...
CHESSClean = getCHESS(paths$chess) %>% chessAdmissionSubset("2020-04-15")
...
symptomaticToAdmission = CHESSClean %>% filter(age>10 & !is.na(estimateddate) & !is.na(hospitaladmissiondate)) %>% generateSurvivalData(
  idVar = caseid,
  startDateVar = estimateddate, 
...
```{r}
uncensoredSymptomaticToAdmission = CHESSClean %>% filter(age>15) %>% generateSurvivalData(
  idVar = caseid,
  startDateVar = estimateddate, 
  endDateExpr = hospitaladmissiondate,
  statusExpr = 1,
...
models = standardModels(c("gamma","weibull", "lnorm"))
...
```{r}
out = uncensoredSymptomaticToAdmission %>% fitModelsByAge(models = standardModels(c("lnorm", "weibull", "gamma")))
```
...
out %>% filter(model=="weibull") %>% ungroup() %>% mutate(value = sprintf("%1.4f [%1.4f; %1.4f]", value, low_ci, if_else(high_ci>100,as.double(NA),high_ci))) %>%
  ...
  ) %>% standardPrintOutput::saveTable("~/Dropbox/covid19/ventilator-demand/parameterisation/symptomatic-to-admission/Table2_exponentialParameterisation")
```
...
surf %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/ventilator-demand/parameterisation/symptomatic-to-admission/Fig4_losDensity")
...
(ggplot(survSurf %>% filter(model=="gamma") %>% mutate(median_lo = ifelse(is.na(median_lo),0,median_lo)),aes(x=ageCat,y=median,ymin=median_lo,ymax=median_hi, group=1))+geom_line(stat="smooth")+geom_ribbon(alpha=0.2)+xlab("age")+ylab("median duration symptoms before admission")) %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/ventilator-demand/parameterisation/symptomatic-to-admission/Fig5_losMedianByAge")

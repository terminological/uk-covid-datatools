...
currentDataset = tsp$getDaily(id = "CURRENT-DATASET", orElse=function() {
  ...
  ### Cases - CTRY ----
  casesCTRY = bind_rows(
    tmp4Nations %>% filter(statistic == "case" & codeType == "CTRY"  & type == "incidence") %>% dpc$demog$findDemographics(),
  ...
  admissionsCTRY = bind_rows(
    tmp4Nations %>% filter(statistic == "hospital admission" & name == "Northern Ireland" & type == "incidence"),
    dpc$spim$getSPIMextract() %>% 
      filter(name == "England" & type == "incidence" & statistic =="hospital admission" & source %in% c("hospital_inc","hospital_inc_new") & is.na(ageCat)) %>% 
    ...
    dpc$spim$getSPIMextract() %>% filter(name == "Scotland" & type == "incidence" & statistic =="hospital admission" & source=="positive_admissions_inpatients")
  ) %>% dpc$demog$findDemographics()

  finalAdmissionsCTRY = admissionsCTRY %>% filter((!name %in% c("Wales","Scotland")) | source != "phe api") %>% mutate(source="mixed api and spim")
  rationale(codeType = "CTRY","hospital admission",
            "PHE api for Englands, spim_hosp_inc_new_authorisation_date field for Wales",
            "None")
  ...
  deathsCTRY = tmp4Nations %>%
    filter(statistic=="death" & codeType == "CTRY"  & type == "incidence") %>%
    dpc$demog$findDemographics()
  ...
  sariIcuAdmissionsNHSER = tmp3 %>% filter(type=="incidence" & statistic=="icu admission" & date > "2020-03-15")

  admissionsNHSER = bind_rows(
      nhserApi %>% filter(statistic =="hospital admission" & codeType == "NHSER"  & type == "incidence"),
  ...
  finalAdmissionsNHSER = admissionsNHSER %>%
    ...
    filter(source %in% c("hospital_inc","hospital_inc_new") & is.na(ageCat)) %>%
    tsp$aggregateSource(list(admissions = c("hospital_inc","hospital_inc_new")), fn=sum) 
  ...
  icuAdmissionsNHSER = bind_rows(
      dpc$spim$getSPIMextract() %>% filter(source %in% c("chess_icu_admissions") & is.na(ageCat) & codeType=="NHSER"),
      sariIcuAdmissionsNHSER
    ) %>%
    dpc$demog$findDemographics()
  finalIcuAdmissionsNHSER = icuAdmissionsNHSER %>% filter(source=="sari summary")
...
#   currentRtQuick
# )
offsets = tsp$defaultOffsetAssumptions()
...
currentRtSlow = tsp$getDaily(id = "CURRENT-RT-SLOW", orElse=function() {
  ...
  with(currentDataset, {
    ...
    final14DayRtDataset = finalDataset %>%
      ...
      tsp$adjustRtDates(offsetAssumptions = offsets) %>%
      tsp$adjustGrowthRateDates(offsetAssumptions = offsets) %>%
      tsp$estimateVolatilty(valueVar = `Mean(R)`)
...
currentRtJEpidemic = tsp$getDaily(id = "CURRENT-RT-JEPIDEMIC", orElse=function() {
  ...
  with(currentDataset, {
    ...
    unsmoothed7DayDataset = finalDataset %>%
      filter(statistic %in% c("case","hospital admission","death")) %>%
      tsp$estimateGrowthRate2(window = 21) %>% 
      mutate(
    ...
    #estim$selectSpecificWindow(window = 7)
    estim$selectAdaptiveWindow(incidenceSum = 100,minWindow = 7)
    ...
    jepidem = estim$estimateRt(unsmoothed7DayDataset,dateColName = "date", incidenceColName = "Imputed.value")

    unsmoothed7DayRtDataset = unsmoothed7DayDataset %>% 
      left_join(jepidem %>% mutate(date=Rt.EndDate), by = c(covidStandardJoins()), suffix=c("",".R")) %>%
      tsp$estimateVolatilty(valueVar = `Mean(R)`) %>%
      mutate(Anomaly.R = Anomaly)

    
    estim2 = J$CoriEstimator$new(r0Mean = 5,r0SD = 5,maxWindow = 7)
    estim2$withInfectivityProfileMatrix(dpc$serial$getBasicConfig(quick = FALSE)$si_sample[,1:10])
    estim2$withAdaptivePrior(factor = 1.25)
    estim2$selectSpecificWindow(window = 7)
    estim2$collectMixtureQuantiles()
    estim2$inMiddleOfTimeseries()
    estim2$legacySupport(TRUE)
    jepidem2 = estim2$estimateRtFromRates(unsmoothed7DayDataset,dateColName = "date", rateColName = "Est.value",samples = 10)
    
    smoothed7DayRtDataset = unsmoothed7DayDataset %>% 
      left_join(jepidem2 %>% mutate(date=Rt.EndDate), by = c(covidStandardJoins()), suffix=c("",".R")) %>%
      tsp$estimateVolatilty(valueVar = `Mean(R)`) %>%
      mutate(Anomaly.R = Anomaly)
    
        
    return(list(
      jepi7 = unsmoothed7DayRtDataset %>%
          tsp$adjustRtDates(offsetAssumptions = offsets) %>%
          tsp$adjustGrowthRateDates(offsetAssumptions = offsets),
      jepiSmooth = smoothed7DayRtDataset %>%
        tsp$adjustRtDates(offsetAssumptions = offsets) %>%
        tsp$adjustGrowthRateDates(offsetAssumptions = offsets)
      )
...
# ukcovidtools::reload()
# 
# unsmoothed7DayDataset = currentDataset$finalDataset %>%
#   filter(statistic %in% c("case","hospital admission","death")) %>%
#   tsp$estimateGrowthRate2(window = 14, nocache=TRUE) %>%
#   mutate(
#     Imputed.value = ifelse(is.na(value), Est.value, value)
#   )  %>%
#   covidStandardGrouping()
...
# estim$legacySupport(TRUE)
# 
# #J$changeLogLevel(logLevel = "DEBUG")
# jepidem = estim$estimateRt(
#   unsmoothed7DayDataset,
#   dateColName = "date", incidenceColName = "Imputed.value")
# 
# unsmoothed7DayRtDataset = unsmoothed7DayDataset %>% 
#   left_join(jepidem %>% mutate(date=Rt.EndDate), by = c(covidStandardJoins()), suffix=c("",".R")) %>%
#   tsp$estimateVolatilty(valueVar = `Mean(R)`) %>%
#   mutate(Anomaly.R = Anomaly)
# 
# currentRtJEpidemic = list(jepi7 = unsmoothed7DayRtDataset)
# 
# tmp1 = currentRtJEpidemic$jepi7 %>% filter(statistic == "case" & name =="North West")
# tmp2 = currentDataset$finalDataset %>% filter(statistic == "case" & name =="North West")

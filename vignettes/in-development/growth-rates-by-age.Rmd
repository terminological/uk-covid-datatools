...
* Size of this outbreak is potentially an underestimate due to testing capacity issues.
...
* During second lockdown all age groups start to decline after 1 week delay, but effect far larger in adult age groups versus under 18.

```{r}
p2 = combined %>% filter(codeType=="CTRY") %>% tsp$plotWindowedGrowthRate(dates = "2020-08-01", events = events2, colour=statistic, rlim = c(-0.1,0.2), leftSided=TRUE) + facet_wrap(vars(ageCat))
...
* In mid to late august growth became positive in most age groups, university age groups are seen to be growing from as early as the beginning of August.
  * True peak of growth rate in younger age groups is unknown as potentially affected by testing capacity issues. 
  * Older age groups case growth is mirrorer by death statistics, suggesting the ascertainment bias due to saturation of test availability has not affected this group.
* University age groups peaked in late September,and largely in negative growth since, expect for prior to 2nd lockdown
* Post second lockdown drop in growth rates in all age groups but less marked in school age groups
  * Effect of half term versus lockdown unclear in these age groups
...
tmp %>% tsp$plotWindowedGrowthRate(dates = "2020-08-01", events = events3, group=name, rlim = c(-0.1,0.2), colour="grey60") +
  ...
  plotRibbons(data = tmp %>% filter(name=="London"),meanVar = `Growth.windowed.value`,sdVar = `Growth.windowed.SE.value`,colourExpr = "blue") +
...
`r cap$fig("growthNHSER","Exponential growth rates of cases in various age groups in the different NHS regions in England. Red is the North West, Blue is London. Other regions not highlighted. Growth rates are shifted in time to align to estimated date of infection.")`
...
* For example the current flat growth in secondary schools in England belies significant differences between London and the rest of the country.
* Growth in school ages initially very much higher in North West compared to South West until late September, when other regions begun to grow faster.
* Growth rate in North West now seen to be negative in all age groups.
* Growth rate has slowed since early to mid October in all regions, but trend is to the increase particularly in younger age groups.
* In the most vulnerable age groups growth currently negative, or close to zero in all regions. 
...
maxDate = (tmp3 %>% pull(date) %>% max())
#plotDates = seq(maxDate,by = -14,length.out = 4)

(tmp3 %>% filter(code != "E99999999") %>% group_by(name,ageCat) %>% tsp$plotGrowthIncidence(plotDates = maxDate,timespan = 31, populationExpr = population, colour = ageCat,maxSize = 4))+scale_color_manual(values = highlightList(vector=tmp3$ageCat,list("75+"="dark magenta","11-17"="#008000","6-10"="#00B000")))+facet_wrap(vars(name))+
  xlab("7 day growth rate")+ylab("Symptomatic positives per 1M per day")

#+facet_grid(cols=vars(plotDate),rows = vars(name))
```

`r cap$fig("growthVsIncidenceNHSER",paste0("Growth versus incidence per capita in different age groups on ",maxDate,". Green highlighting shows the school age groups, magenta the most vulnerable elderly."))`

* Regional differences suggest current growth in South East and East of England and London has turned a corner in all age groups, and moved from negative to strong positive growth. This has been lead by school age groups.
* Midlands, North East, and North west and South West rates all suggest a trend to a return to positive growth but no strong evidence this is isolated to any specific age groups. If anything this is more in elderly than school age.
...
```{r}
p3 = ggplot(map %>% ungroup() %>% sf::st_as_sf()) + geom_sf(aes(fill=Growth.windowed.value),size = 0.05) + facet_wrap(vars(ageCat), ncol = 3) + standardPrintOutput::defaultMapLayout() + standardPrintOutput::narrower() + scale_fill_distiller(palette = "RdGy", name="growth rate", oob=scales::squish, lim=c(-0.10,0.10))
...
```{r}
p4 = ggplot(map %>% ungroup() %>% filter(ageCat=="all") %>% sf::st_as_sf()) + geom_sf(aes(fill=Growth.windowed.value),size = 0.05) + standardPrintOutput::defaultMapLayout() + standardPrintOutput::narrower() + scale_fill_distiller(palette = "RdGy", name="growth rate", oob=scales::squish, lim=c(-0.10,0.10))
p4 %>% standardPrintOutput::saveFullPageFigure("~/Dropbox/covid19/by-age/CaseGrowthByLAD") %>% invisible()
```

```{r}
p5 = ggplot(map %>% ungroup() %>% sf::st_as_sf()) + geom_sf(aes(fill=Growth.windowed.ProbPos.value),size = 0.05) + facet_wrap(vars(ageCat), ncol = 3) + standardPrintOutput::defaultMapLayout() + standardPrintOutput::narrower() + scale_fill_distiller(palette = "RdYlGn", name="P(r>0)",lim = c(0,1)) #, oob=scales::squish, lim=c(-0.10,0.10))
p5 %>% standardPrintOutput::saveTwoThirdPageFigure("~/Dropbox/covid19/by-age/CasePGrowthPosByAgeAndLAD")
```


`r cap$fig("PgrowthPosLAD",paste0("Probability of positive exponential growth rates of symptomatic cases by local authority district in functional age categories as of ",maxDate,". For clarity several age groups have been combined. School age comprises 6-17 year olds, University is from 18-22. Other family is 0-6 and 23-59, Retirement is 60 and over"))`
...
```{r}
ggplot(totalCases2 %>% filter(name!="England" & date > "2020-03-15"),aes(x=date,y=percentAllAges*100,fill=ageCat)) + geom_area()+facet_wrap(vars(name))+ylab("%age of total") + scale_x_date(date_breaks = "4 weeks",date_labels = "%d-%m")+tsp$plotEvents(events4,labelSize = 8,labelY=75)+facet_wrap(vars(name))+narrowAndTall()
```

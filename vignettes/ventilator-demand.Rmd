...
applyAgeTimeMatrix = function(inputDf, matrixDf, 
  ...
  outputDf = inputDf %>% left_join(matrixDf, by=ageCatVar) %>% 
    mutate(
      !!newDateVar := !!oldDateVar+days,
      !!newIncidenceVar := !!oldIncidenceVar*probability
    ) %>%
    group_by(
      trustId,ageSpecificPopulation,regionId,trustName,acuteBeds,
      dayBeds,icuBeds,approxLat,approxLong, !!newDateVar, !!ageCatGrp
    ) %>%
    summarise (
...
processModelResult = function(infected, trustList=c("RBA","RBZ","RH8","RRK")) {
  infected = infected %>% filter(trustId %in% trustList)
  simEnd = max(infected$infectedDate)
  # Transform infected to symptomatic
  # N.B. This filters out a large fraction of the young, as they have mild symptoms.
  # It also introduces a lag because of the incubation period

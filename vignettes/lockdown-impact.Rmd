...
keyDates = tibble(
  date = c(as.Date(c("2020-03-13","2020-03-16","2020-03-19","2020-03-23")), max(R0regionaltimeseries$date, na.rm=TRUE)),
  impactDate = c(as.Date(c("2020-03-14","2020-03-21","2020-03-24","2020-03-28")), max(R0regionaltimeseries$date, na.rm=TRUE)),
...
r0shapes = r0shapes %>% mutate(`Median(R)` = ifelse(`Median(R)`>5, 4.999,`Median(R)`))
...
```{r}
rateOfChange %>% ungroup() %>% filter(slopeLowerCi > 0) %>% arrange(desc(slope)) %>% head(5) %>% mutate(`95% CI` = sprintf("%1.2f; %1.2f",slopeLowerCi,slopeUpperCi)) %>% select(`Unitary authority`=GSS_NM,`dR/dt`=slope, `95% CI` ,`R^2`=r_squared) %>% standardPrintOutput::saveTable("~/Dropbox/covid19/lockdown-impact/Top5UnitaryAuthoritiesByDeltaR_t")
```

```{r}
rateOfChange %>% ungroup() %>% filter(slopeUpperCi < 0) %>% arrange(slope) %>% head(5) %>% mutate(`95% CI` = sprintf("%1.2f; %1.2f",slopeLowerCi,slopeUpperCi)) %>% select(`Unitary authority`=GSS_NM,`dR/dt`=slope, `95% CI` ,`R^2`=r_squared) %>% standardPrintOutput::saveTable("~/Dropbox/covid19/lockdown-impact/Bottom5UnitaryAuthoritiesByDeltaR_t")
```
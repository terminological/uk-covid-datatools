...
table %>% group_by(`Reference`) %>% standardPrintOutput::saveTable("~/Dropbox/covid19/lockdown-impact/Table1_serialIntervals", defaultFontSize = 8, colWidths=c(4.5,2,2,0.5,1))
```
...
ukregionalplot %>% standardPrintOutput::saveHalfPageFigure("~/Dropbox/covid19/lockdown-impact/Fig1_ukRegionRt")
```
...
ukwide %>% standardPrintOutput::saveThirdPageFigure("~/Dropbox/covid19/lockdown-impact/Fig3_englandMap")
...
timeGif = gganimate::animate(barAnim, renderer=gganimate::magick_renderer(), width=400, height=60)

anim = ukwide+guides(fill="none")+#labs(title = 'Date: {frame_time}')+
  gganimate::transition_time(date)
gif = gganimate::animate(anim, renderer=gganimate::magick_renderer(), duration = 10, fps=10, width=400, height=800)
...
gif2 = gganimate::animate(anim2, renderer=gganimate::magick_renderer(), duration = 10, fps=10, width=400,height=300)
#gganimate::anim_save("~/Git/uk-covid-datatools/vignettes/London_Rt_by_unitary_authority_over_time.gif",gif2)

#a_mgif <- magick::image_read(gif)
#b_mgif <- magick::image_read(gif2)
new_gif <- magick::image_append(c(gif[1], gif2[1], timeGif[1]), stack=TRUE)
for(i in 2:100){
  combined <- magick::image_append(c(gif[i], gif2[i], timeGif[i]), stack=TRUE)
...
* https://github.com/terminological/uk-covid-datatools/blob/master/vignettes/delta_Rt_over_time.gif
...
gif3 = gganimate::animate(anim3, renderer=gganimate::magick_renderer(), width=300, height = 300)
...
gif4 = gganimate::animate(anim4, renderer=gganimate::magick_renderer(), width=500, height = 300)

new_gif <- magick::image_append(c(magick::image_append(c(gif3[1], gif4[1])),timeGif[1]),stack=TRUE) 
for(i in 2:100){
  combined <- magick::image_append(c(magick::image_append(c(gif3[i], gif4[i])),timeGif[i]),stack=TRUE) 
  new_gif <- c(new_gif, combined)
}

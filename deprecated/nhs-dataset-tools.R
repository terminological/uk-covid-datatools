...
#'     `Day of Value` = df %>% dplyr::pull(!!dateVar) %>% dplyr::format("%d") %>% as.integer(),
#'     `Month of Value` = df %>% dplyr::pull(!!dateVar) %>% dplyr::format("%m") %>% as.integer(),
#'     `Year of Value` = df %>% dplyr::pull(!!dateVar) %>% dplyr::format("%Y") %>% as.integer(),
#'     `Geography` = df %>% dplyr::mutate(geo_out = !!geographyExpr) %>% dplyr::pull(geo_out),
#'     `ValueType` = "R",
#'     `Quantile 0.05` = df %>% dplyr::pull(`Quantile.0.05(R)`) %>% round(2),
#'     `Quantile 0.1` = NA,
#'     `Quantile 0.15` = NA,	
#'     `Quantile 0.2` = NA,	
#'     `Quantile 0.25` = df %>% dplyr::pull(`Quantile.0.25(R)`) %>% round(2),
#'     `Quantile 0.35` = NA,	
#'     `Quantile 0.4` = NA,	
#'     `Quantile 0.45` = NA,
#'     `Quantile 0.5` = df %>% dplyr::pull(`Median(R)`) %>% round(2),
#'     `Quantile 0.55` = NA,
#'     `Quantile 0.6` = NA,	
#'     `Quantile 0.65` = NA,	
#'     `Quantile 0.7` = NA,	
#'     `Quantile 0.75` = df %>% dplyr::pull(`Quantile.0.75(R)`) %>% round(2),
#'     `Quantile 0.8` = NA,	
#'     `Quantile 0.85` = NA,	
#'     `Quantile 0.9` = NA,	
#'     `Quantile 0.95` = df %>% dplyr::pull(`Quantile.0.95(R)`) %>% round(2),
#'   ))
#' },
#' 
